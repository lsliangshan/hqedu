        ...
        <div class="form_inner">
          <div class="form_wrapper register_login_form">
            <Form :model="formData" :rules="formRules" ref="formRef">
              <FormItem v-for="(fi, index) in pageData.formItems" :key="index">
                <select v-if="fi.type.toLowerCase() === 'select'" size="large" class="query_custom_select" style="height: 40px; line-height: 40px;" v-model="fi.defaultValue" :placeholder="fi.placeholder">
                  <option v-for="(o, idx) in fi.data" :key="idx" :value="o.value">{{o.text}}</option>
                </select>
                <span v-if="fi.type.toLowerCase() === 'select' && !fi.defaultValue" class="query_custom_select_placeholder">{{fi.placeholder}}</span>
                <Icon type="arrow-down-b" size="18" v-if="fi.type.toLowerCase() === 'select'" class="query_custom_select_arrow_down"></Icon>
              ...
              <FormItem prop="password">
                <Input type="password" v-model="formData.password" placeholder="请输入密码" @keyup.13="submit"/>
              </FormItem>
              <FormItem prop="code">
...
<style scoped>
  ...
  .query_custom_select_placeholder {
    color: #bbbec4;
    position: absolute;
    z-index: 9;
    height: 40px;
  ...
  .query_custom_select_arrow_down {
    position: absolute;
    right: 10px;
    top: 0;
    height: 40px;
